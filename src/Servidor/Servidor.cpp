...
void Servidor::handshake(mensajeStruct msg){
    ...
    this->escenario->velocidadBala = velocidadBalaAux;
    cout << " VELOCIDAD DE BALA " << velocidadBalaAux;
...
*/
	...
	if (this->escenario->bonusInactivos.size() == 0){
		this->escenario->bonusInactivos = parser->getBonuses();
	}

	if (this->escenario->enemigosInactivos.size() == 0){
		std::vector<enemigoStruct> enemigos = parser->getListaEnemigos();
	...
	if (this->escenario->PlataformasInactivos.size() == 0){
		...
		//	int posYP = atoi(result[3].c_str());
			Plataforma *plataforma = new Plataforma(objectId5,imagen,5,posXP,anchoP,altoP, parser->getAltoEscenario(), &this->escenario->mapJugadores);
